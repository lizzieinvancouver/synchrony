...
# it looks like the phenovalues and year were copied (and added) from spp2 to spp1
# and from spp1 to spp2. For example, Acartia tonsa (type spp1) has data from
# both spp1 and spp2 but all labeled as Acartia tonsa and spp1"
...
names(spp1or2melt) <- c("intid", "interaction", "type", "species")
spp1or2.nodup <- spp1or2melt[!(duplicated(spp1or2melt$species)==TRUE),] # get only one species total
rawlongwtype <- merge(rawlong, spp1or2.nodup, by="species", all.x=TRUE)
rawlongwtype$X <- NULL # delete old row numbers

write.csv(rawlongwtype, "output/rawlongwtype.csv", row.names=FALSE)
write.csv(spp1or2.nodup, "output/speciesdata.csv", row.names=FALSE)
