...
// But updated by Lizzie to try to get just predictions to run
// following along on pages 65-66 of Stan 2.6.2 to work on getting generated quantities

data {
  int N;                                  // # data points
  int N_new;				  // # for predictions
  real y[N];
  int J;                                  // # species
  int species[N];
  real year[N];
...
generated quantities { 
  ...
  for (n in 1:N_new){
    ...
    s <- species[n];
    t <- type[s];
  ypred2[n] <- level[t] + sigma_level[t]*species_level[s] +
                  (trend[t] + sigma_trend[t]*species_trend[s])*year[n];
}
